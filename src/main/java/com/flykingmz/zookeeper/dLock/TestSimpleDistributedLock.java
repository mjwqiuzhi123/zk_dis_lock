...
public class TestSimpleDistributedLock {
	...
	public static void main(String[] args) throws InterruptedException {
		final DistributedLock dc = new SimpleDistributedLock("39.105.8.31:2181,39.105.8.31:2182,39.105.8.31:2183","/rootLock");  
		 for(int i=0; i < THREAD_NUM; i++){  
	            new Thread(){  
	                @Override  
	                public void run() {  
	                    try{  
	                        logger.info("thread start now "+Thread.currentThread().getId());
	                        dc.lock();
	                        dc.unLock();
